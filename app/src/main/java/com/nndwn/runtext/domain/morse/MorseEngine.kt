...
/**
 * Represents a single timing element in International Morse Code. Each element has a [durationMultiplier] relative to
 * the base unit duration.
 *
 * ITU standard timing: Dot = 1 unit, Dash = 3 units Intra-char gap = 1 unit, Char gap = 3 units, Word gap = 7 units
 */
...
object MorseEngine {
  ...
  private val LATIN_MORSE_MAP: Map<Char, String> =
    mapOf(
      'A' to ".-",
      'B' to "-...",
      'C' to "-.-.",
  ...
  private val HANGUL_JAMO_MORSE_MAP: Map<Char, String> =
    mapOf(
  ...
  /**
   * Convert [text] to a list of [MorseElement] with proper inter-symbol, inter-character, and inter-word gaps. Unknown
  ...
  private fun getMorseStringForChar(char: Char): List<String> {
    ...
    LATIN_MORSE_MAP[upperChar]?.let {
      return listOf(it)
    }

    HANGUL_JAMO_MORSE_MAP[char]?.let {
      return it.split(' ')
    }
  ...
  private fun decompoundJongSeong(jong: Char): List<Char> =
    when (jong) {
      'ㄳ' -> listOf('ㄱ', 'ㅅ')
      'ㄵ' -> listOf('ㄴ', 'ㅈ')
      'ㄶ' -> listOf('ㄴ', 'ㅎ')
  ...
  /**
   * Calculate the duration of one Morse unit in milliseconds. Based on the PARIS standard: "PARIS" = 50 units → 1 unit
   * = 1200/wpm ms.
   */
  ...
  fun isSignalElement(element: MorseElement): Boolean = element is MorseElement.Dot || element is MorseElement.Dash
}
